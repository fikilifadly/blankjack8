...
const stand = () => {
  ...
  while (dealer.cardSum < 17) {
    //dealer should have at least 17
    distributeCard(dealer);
    reduceAce(dealer);
  }
  checkWinner();
...
const popUpLogin = () => {
  rulesbtn.click();
  let chips = document.querySelector(".chips-balance").innerText.replace("$", "");
...
if (currchips && name) {
  leaderboard.push([name, currchips]);
}

// Sort the leaderboard array in descending order based on the second element (scores)
leaderboard.sort((a, b) => b[1] - a[1]);

let leaderboardEl = document.querySelector(".data-leader ol");
